...
#define FL "563790_563910_4193790_4193910.txt"
#define LN_NUMBER 14212054
#define NB_THREAD 32
...
string local_labels, local_points;
...
class BigCube{
	...
	{	
		...
		while(cy < 10)
		{
			...
			while(cx < 10)
			{
				...
				while(cz < 10)
				{
					...
					labels.clear();
					//if(avg!=0) cout<<to_string(avg)<<endl;
			...
			//cout <<setprecision(10) << "y=" << y <<endl;
			y=inc+y;
			//cout <<setprecision(10) <<  "After inc: y=" << y << " by=" << by << " inc=" << inc << endl;
		}
		//cout << "cx= " << cx << " cy= " << cy << " cz= " << cz << endl;
		max = 0;
		most_common = "null";
		...
		write_mutex.lock();
		local_points = local_points + output_line;
		//points_file.flush();
		//labels_file << lbl << "\n";
		local_labels = local_labels + lbl;
...
void getMinMax()
{
	...
	//cout << "Begin! " << "currentX= " << currentX << " currentY= " << currentY << " currentZ= " << currentZ << endl;
}
...
void writer_thread()
{
	while(!completed)
	{
		this_thread::sleep_for(chrono::minutes(WRITE_TIME));
		write_data();
	}
	write_data();
}
...
int main()
{
	...
	thread write_thread(writer_thread);
